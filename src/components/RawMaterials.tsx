...
import { fetchRawMaterials, addRawMaterial, updateRawMaterial, deleteRawMaterial } from '../utils/api';

const emptyForm = { name: '', unit: 'کیلو', quantity: 0, price_per_unit: 0, supplier: '', min_stock: 0 };
...
export const RawMaterials: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    if (editId) {
      await updateRawMaterial(editId, form);
    } else {
  ...
  const handleEdit = (item: RawMaterial) => {
    setEditId(item.id);
    setForm({ name: item.name, unit: item.unit, quantity: item.quantity, price_per_unit: item.price_per_unit, supplier: item.supplier, min_stock: item.min_stock });
    setShowModal(true);
  };
  ...
  const filtered = items.filter((i) => i.name.includes(search) || i.supplier.includes(search));
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-bold">مواد اولیه</h2>
      ...
      <div className="overflow-x-auto">
        <table className="table table-zebra">
          <thead>
            <tr>
              <th>#</th><th>نام</th><th>واحد</th><th>موجودی</th><th>قیمت واحد</th><th>تأمین‌کننده</th><th>حداقل</th><th>عملیات</th>
            </tr>
          </thead>
          <tbody>
      ...
      {showModal && (
        <div className="modal modal-open">
          <div className="modal-box">
            ...
            <h3 className="font-bold text-lg mb-4">{editId ? 'ویرایش ماده اولیه' : 'افزودن ماده اولیه'}</h3>
            <div className="space-y-3">
              ...
              <select className="select select-bordered w-full" value={form.unit} onChange={(e) => setForm({ ...form, unit: e.target.value })}>
                <option>کیلو</option><option>متر</option><option>لیتر</option><option>عدد</option><option>بسته</option>
              </select>
              ...
              <input className="input input-bordered w-full" placeholder="تأمین‌کننده" value={form.supplier} onChange={(e) => setForm({ ...form, supplier: e.target.value })} />
